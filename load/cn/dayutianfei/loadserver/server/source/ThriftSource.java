...
public class ThriftSource extends AbstractSource implements Configurable,
	...
	public void configure(Context context) {
		...
		Preconditions.checkNotNull(bindAddress,
				"Bind address must be specified " + "for Thrift Source.");

		try {
			maxThreads = context.getInteger(CONFIG_THREADS, 0);
		} catch (NumberFormatException e) {
			LOG.warn("Thrift source\'s \"threads\" property must specify an "
    ...
    @Override
	public void start() {
		LOG.info("Starting thrift source");
		...
		try {
			...
			ThreadFactory threadFactory = new ThreadFactoryBuilder()
					.setNameFormat("Flume Thrift IPC Thread %d").build();
			if (maxThreads == 0) {
			...
			LOG.info("TThreadedSelectorServer not found, "
			...
			try {
				...
				throw new FlumeException(
						"Cannot find TThreadSelectorServer or "
		...
		try {
			 ...
			 * method does not exist in the parent abstract Args class, so use
			 * reflection to pass the executor in.
			 */
		...
		servingExecutor = Executors
				.newSingleThreadExecutor(new ThreadFactoryBuilder()
						.setNameFormat("Flume Thrift Source I/O Boss").build());
		...
		sourceCounter.start();
		LOG.info("Started Thrift source.");
		super.start();
	}
	...
	private class ThriftSourceHandler implements ThriftSourceProtocol.Iface {
		...
		public Status append(ThriftFlumeEvent event) throws TException {
			...
			try {
				...
				LOG.warn("Thrift source " + getName()
						+ " could not append events " + "to the channel.", ex);
		...
		public Status appendBatch(List<ThriftFlumeEvent> events)
			...
			try {
				...
				LOG.warn("Thrift source %s could not append events to the "
						+ "channel.", getName());
				return Status.FAILED;
			}
		...
		public String getSchema(String dbName, String tblName)
				throws TException {
			return ClientCache.getTableInfo(dbName, tblName).toSchema();
		}

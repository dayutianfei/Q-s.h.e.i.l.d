...
import cn.dayutianfei.loadserver.server.load.FileManager;
import cn.dayutianfei.loadserver.util.DataUtil;
 ...
 *
 */
public class EventSinkRC extends AbstractSink implements Configurable {
	
	private static final Logger LOG = LoggerFactory.getLogger(EventSinkRC.class);
	
	private long totalDealNumber = 0;
	private DataDeserialization dataAssist = null;
	private FileManager fileManager = null;
	private RCFileWriter writer = null;
...
//                return Status.READY;
            ...
            totalDealNumber += num;
            LOG.info("insert data number " + num + ", total is " + totalDealNumber);
            LOG.info("start to writer file");
            writer = fileManager.getWriter("test-test1");
            for(List<Object> line : lines){
                writer.append(DataUtil.objectList2StringArray(line, ClientCache.getTableInfo(dbName, tblName)));
            }
            transaction.commit();
            return Status.READY;
        }

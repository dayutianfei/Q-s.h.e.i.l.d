...
public class MakingManySmallFilesOnHDFS {
	private static final Log LOG = LogFactory.getLog(MakingManySmallFilesOnHDFS.class);
	...
	public static void main(String[] args) {
		...
		for(int i = 0 ; i < subDirectoryNumber; i ++){
			...
			for(long j = 0 ; j< fileNumberInEachDir; j++){
			    ...
			    r.nextBytes(nbyte);
			    try {
					createFile(subDirPath+"file-"+j,nbyte);
				} catch (IOException e) {
					e.printStackTrace();
				}
			    totalFileNumber++;

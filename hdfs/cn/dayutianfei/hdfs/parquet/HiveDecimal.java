...
/**
 * HiveDecimal. Simple wrapper for BigDecimal. Adds fixed max precision and non
 * scientific string representation
 */
public class HiveDecimal implements Comparable<HiveDecimal> {
	 ...
	 * as in case of a non-generic udf.
	 */
	 ...
	 * which is equivalent to SQL decimal precision. Note that this is different
	 * from BigDecimal.precision(), which returns the precision of the unscaled
	 * value (BigDecimal.valueOf(0.01).precision() = 1, whereas
	 * HiveDecimal.create("0.01").precision() = 2). If you want the BigDecimal
	 * precision, use HiveDecimal.bigDecimalValue().precision()
	 * 
	 * @return
	 */
	public int precision() {
		...
		if (bdPrecision < bdScale) {
			...
			// In this case, we'll set the type to have the same precision as
			// the scale.
			return bdScale;
		}
		return bdPrecision;
	...
	private static BigDecimal normalize(BigDecimal bd, boolean allowRounding) {
		...
		int maxScale = Math.min(MAX_SCALE,
				Math.min(MAX_PRECISION - intDigits, bd.scale()));
		if (bd.scale() > maxScale) {
			if (allowRounding) {
				bd = bd.setScale(maxScale, RoundingMode.HALF_UP);
				// Trimming is again necessary, because rounding may introduce
				// new trailing 0's.
				bd = trim(bd);
			} else {
				bd = null;
	...
	public static BigDecimal enforcePrecisionScale(BigDecimal bd,

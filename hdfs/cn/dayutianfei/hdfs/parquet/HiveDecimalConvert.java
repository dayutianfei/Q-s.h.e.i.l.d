...
public class HiveDecimalConvert implements
		WritableComparable<HiveDecimalConvert> {
	...
	public HiveDecimalConvert() {
	}

	public HiveDecimalConvert(byte[] bytes, int scale) {
		set(bytes, scale);
	}

	public HiveDecimalConvert(HiveDecimalConvert writable) {
		set(writable.getHiveDecimal());
	}

	public HiveDecimalConvert(HiveDecimal value) {
		set(value);
	}
	...
	public void set(HiveDecimalConvert writable) {
		set(writable.getHiveDecimal());
	}
	...
	/**
	 * Get a HiveDecimal instance from the writable and constraint it with
	...
	// return HiveDecimalUtils.enforcePrecisionScale(HiveDecimal.create(new
	// BigInteger(internalStorage), scale),
	...
	@Override
	public int compareTo(HiveDecimalConvert that) {
	...
	public boolean equals(Object other) {
		if (other == null || !(other instanceof HiveDecimalConvert)) {
			return false;
		}
		HiveDecimalConvert bdw = (HiveDecimalConvert) other;

		// 'equals' and 'compareTo' are not compatible with HiveDecimals. We
		// want
	...
	/*
	 * (non-Javadoc) In order to update a Decimal128 fast (w/o allocation) we
	 * need to expose access to the internal storage bytes and scale.
	 * 
	 * @return
	 */

package cn.dayutianfei.common.hash;
...
public class HashUtil {
	 ...
	 * (Xmx:8G) 97867960:201165
	 * (Xmx:8G,hashcode)99562592:148229
	 */
	...
	public static int crc32Hash(byte[] key) {
		int n = key.length;
		int hash = n;
		//System.out.println(crctab.length);
		for (int i = 0; i < n; ++i){
...
//		System.out.println(HashUtil.crc32Hash("Y2VzaWjkuK3mloc="));
//		System.out.println(HashUtil.crc32Hash("1"));
		System.out.println("a".getBytes().toString());
		String test = "p_2_焦煜_1";
		//必须使用UTF-8编码进行文中解析，才能保证和Java版的保持一致
		System.out.println(HashUtil.crc32Hash(test.getBytes("utf8")));
		System.exit(0);
		int targetLength = Integer.parseInt(args[0]);
		System.out.println("current number range is "+targetLength);
		long start = System.currentTimeMillis();
//		HashMap<Integer,Integer> re = new HashMap<Integer,Integer>();
		HashSet<Integer> re = new HashSet<Integer>();
		for(int i=0;i<targetLength;i++){
			String temp = ""+i;
			int _re = HashUtil.crc32Hash(temp);
//			if(re.containsKey(re)){
//				re.put(_re, re.get(_re)+1);
//			}else{
//				re.put(_re, 1);
//			}
			re.add(_re);
		}
		System.out.println(re.size() + " " +(System.currentTimeMillis()-start));
		long start1 = System.currentTimeMillis();
//		HashMap<Integer,Integer> re1 = new HashMap<Integer,Integer>();
		HashSet<Integer> re1 = new HashSet<Integer>();
		for(int i=0;i<targetLength;i++){
			String temp = ""+i;
			int _re = temp.hashCode();
//			if(re1.containsKey(re)){
//				re1.put(_re, re.get(_re)+1);
//			}else{
//				re1.put(_re, 1);
//			}
			re1.add(_re);
		}
		System.out.println(re1.size() + " " +(System.currentTimeMillis()-start1));
	}

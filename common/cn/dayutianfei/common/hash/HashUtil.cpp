...
int crctab[] = { 0x00000000, 0x77073096, 0xee0e612c,
 ...
 */
int crc32Hash(char* key, int length) {
	int n = length;
	int hash = n;
	for (int i = 0; i < n; i++) {
		int pos = (hash & 0xff) ^ key[i];
		if (pos < 0) {
			pos = pos % getArrayLen(crctab) + getArrayLen(crctab);
		}
		if (pos >= getArrayLen(crctab)){
			pos = pos % getArrayLen(crctab);
		}
